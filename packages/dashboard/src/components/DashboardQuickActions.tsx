  ...
  return (
    ...
    >
      ...
      {hoveredAgent && hoveredIdx !== null && (
        ...
        >
          <button
            onClick={() => navigate({ to: '/agents/$agentId', params: { agentId: hoveredAgent.id }, search: { tab: 'persona' } })}
            className="px-2 py-1 text-[11px] font-semibold text-muted-foreground border-b mb-0.5 whitespace-nowrap text-left w-full hover:text-foreground transition-colors"
          >
        ...
        <ul className="max-h-72 overflow-y-auto pb-safe pb-4">
          {agents.map((agent) => (
            <li key={agent.id} className="border-b last:border-b-0">
              <button
                onClick={() => { navigate({ to: '/agents/$agentId', params: { agentId: agent.id }, search: { tab: 'persona' } }); setOpen(false); }}
                className="flex items-center gap-2 px-4 py-2 text-xs font-semibold text-muted-foreground bg-muted/40 w-full text-left hover:text-foreground transition-colors"
              >
