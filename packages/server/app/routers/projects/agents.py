...
async def assign_agent_to_project(
    ...
    """
    logger.debug(
        f"Assigning agent: project_id={project_id}, agent_id={req.agentId}, role={req.role}"
    )
    now = now_ms()
    ...
    result = await session.execute(
        select(ProjectAgentModel).where(
            ...
            ProjectAgentModel.agent_id == req.agentId,
        )
    )
    ...
    await _publish_event(
        "AGENT_ASSIGNED",
        {"projectId": project_id, "agentId": req.agentId, "role": req.role},
    )
...
async def list_project_agents(
    ...
    # Load routine mappings for all agents in this project
    ...
    if agent_ids:
        mappings_result = await session.execute(
            select(ProjectAgentRoutine).where(
                ...
                ProjectAgentRoutine.agent_id.in_(agent_ids),
            )
        )
        for m in mappings_result.scalars().all():
            routine_mappings.setdefault(m.agent_id, []).append(
                {
                    "id": m.id,
    ...
    return [
        {
            ...
            "routine_mappings": routine_mappings.get(a.agent_id, []),
        }
        for a in agents
    ]
...
async def update_agent_role(
    project_id: str,
    agent_id: str,
    req: UpdateAgentRoleRequest,
    session: AsyncSession = Depends(get_async_session),
):
    """Update an agent's role on a project."""
    logger.debug(
        f"Updating agent role: project_id={project_id}, agent_id={agent_id}, new_role={req.role}"
    )
    await get_project_or_404(session, project_id)
    ...
    if not agent:
        raise HTTPException(
            status_code=404, detail=f"Agent {agent_id} is not assigned to this project"
        )
    ...
    await _publish_event(
        "AGENT_ROLE_UPDATED",
        {"projectId": project_id, "agentId": agent_id, "role": req.role},
    )
...
async def remove_agent_from_project(
    project_id: str, agent_id: str, session: AsyncSession = Depends(get_async_session)
):
    """Remove an agent from a project."""
    logger.debug(
        f"Removing agent from project: project_id={project_id}, agent_id={agent_id}"
    )
    await get_project_or_404(session, project_id)

    # Remove routine mappings first (cleanup before removing agent assignment)
    await session.execute(
    ...
    await _publish_event(
        "AGENT_REMOVED", {"projectId": project_id, "agentId": agent_id}
    )
