...
def _git_push(workspace_path: str, branch: str = "main", force: bool = False) -> dict:
    ...
    """
    logger.debug(
        f"Git push: workspace={workspace_path}, branch={branch}, force={force}"
    )
    ...
    try:
        ...
        try:
            ...
            if any(
                phrase in error_msg.lower()
                for phrase in ["rejected", "non-fast-forward"]
            ):
...
async def get_git_status(run_id: str):
    ...
    try:
        ...
        tracking_branch = (
            tracking_result.stdout.strip() if tracking_result.returncode == 0 else None
        )
...
async def get_git_history(run_id: str, limit: int = 50, offset: int = 0):
    ...
    logger.debug(
        f"Getting git history for run_id={run_id}, limit={limit}, offset={offset}"
    )
    ...
    try:
        ...
        return {"run_id": run_id, "commits": commits, "total": len(commits)}
...
async def get_file_at_commit(run_id: str, commit_hash: str, file_path: str):
    ...
    logger.debug(
        f"Getting file at commit for run_id={run_id}, commit={commit_hash}, path={file_path}"
    )
    ...
    try:
        ...
        if (
            ...
            or "exists on disk" in error_msg
            or "fatal: path" in error_msg
        ):
            raise HTTPException(
                status_code=404,
        ...
        if "invalid object" in error_msg or "bad revision" in error_msg:
            raise HTTPException(
                status_code=400, detail=f"Invalid commit hash: {commit_hash}"
            )
        raise HTTPException(status_code=500, detail=f"Git command failed: {e.stderr}")
    except UnicodeDecodeError:
        raise HTTPException(
            status_code=415, detail="Binary file, cannot display as text"
        )
    except Exception as e:
...
async def get_file_history(run_id: str, file_path: str, limit: int = 20):
    ...
    logger.debug(
        f"Getting file history for run_id={run_id}, path={file_path}, limit={limit}"
    )
    ...
    try:
        ...
        if "does not exist" in error_msg or "fatal: ambiguous argument" in error_msg:
            raise HTTPException(
                status_code=404,
                detail=f"File '{file_path}' not found in repository history",
            )
        raise HTTPException(status_code=500, detail=f"Git command failed: {e.stderr}")
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to read file history: {str(e)}"
        )
...
async def merge_run_to_main(run_id: str, request: MergeRequest = MergeRequest()):
    ...
    if strategy not in ["merge", "squash", "rebase"]:
        ...
        raise HTTPException(
            status_code=400,
            detail="Invalid merge strategy. Use: merge, squash, or rebase",
        )
    ...
    try:
        ...
        if "main" not in branches_result.stdout:
            return MergeResponse(success=False, error="Main branch not found")

        # Already on main?
        if current_branch == "main":
            return MergeResponse(
                success=False, error="Already on main branch, nothing to merge"
            )
        ...
        try:
            ...
            return MergeResponse(success=False, error="Failed to check merge status")
        ...
        try:
            ...
            return MergeResponse(success=True, commit_hash=commit_hash)
            ...
            if conflict_files:
                ...
                try:
                    if strategy == "rebase":
                        subprocess.run(
                            ["git", "rebase", "--abort"], cwd=base, capture_output=True
                        )
                    else:
                        subprocess.run(
                            ["git", "merge", "--abort"], cwd=base, capture_output=True
                        )
                except:
                    pass
            ...
            return MergeResponse(success=False, error=f"Merge failed: {error_msg}")

    except subprocess.CalledProcessError as e:
        error_msg = e.stderr if e.stderr else str(e)
        raise HTTPException(
            status_code=500, detail=f"Git operation failed: {error_msg}"
        )
    except Exception as e:
...
async def push_to_remote(run_id: str, request: PushRequest = PushRequest()):
    ...
    """
    logger.debug(
        f"Pushing run_id={run_id} to remote, branch={request.branch}, force={request.force}"
    )
...
async def push_project_to_remote(project_id: str, request: PushRequest = PushRequest()):
    ...
    """
    logger.debug(
        f"Pushing project_id={project_id} to remote, branch={request.branch}, force={request.force}"
    )

    # Project workspaces are in /jfs/workspaces/{project_id}
