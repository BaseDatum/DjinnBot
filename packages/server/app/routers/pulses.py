...
def _get_pulse_schedule(agent_id: str) -> PulseScheduleConfig:
    ...
    for b in raw_blackouts:
        blackouts.append(PulseBlackout(
...
def _compute_upcoming_pulses(agent_id: str, schedule: PulseScheduleConfig, hours: int = 24) -> List[ScheduledPulse]:
    """Compute upcoming pulses for an agent."""
...
def _detect_conflicts(pulses: List[ScheduledPulse], window_ms: int = 120000) -> List[PulseConflict]:
    ...
    for i, pulse in enumerate(pulses):
        ...
        conflicting = [p for p in pulses if 
                      p.scheduledAt >= window_start and 
                      p.scheduledAt < window_end and
                      p.agentId != pulse.agentId]
        
        if conflicting:
            # Create window key to avoid duplicates
            window_key = (window_start // window_ms, frozenset(p.agentId for p in [pulse] + conflicting))
            if window_key in seen_windows:
                continue
            seen_windows.add(window_key)
            
            agents = [{'agentId': pulse.agentId, 'scheduledAt': pulse.scheduledAt, 'source': pulse.source}]
            for p in conflicting:
                agents.append({'agentId': p.agentId, 'scheduledAt': p.scheduledAt, 'source': p.source})
            
            conflicts.append(PulseConflict(
...
@router.get("/timeline")
async def get_pulse_timeline(hours: int = Query(24, ge=1, le=168)):
...
async def add_one_off_pulse(agent_id: str, req: OneOffPulseRequest):
    ...
    try:
        ...
        if pulse_time < datetime.utcnow():
            raise HTTPException(status_code=400, detail="Pulse time must be in the future")
    except ValueError:
