...
async def get_agent_lifecycle(agent_id: str):
    ...
    logger.debug(f"Lifecycle state for agent_id={agent_id}: state={state.get('state')}, queueDepth={queue_depth}")
...
async def get_agent_activity(
    ...
    """
    logger.debug(f"Getting activity for agent_id={agent_id}, limit={limit}, since={since}")
...
async def get_agent_pulse_status(agent_id: str):
    ...
    pulse_enabled = agent_config.get('pulse_enabled', agent_config.get('pulseEnabled', True))
    pulse_interval = agent_config.get('pulse_interval_minutes', agent_config.get('pulseIntervalMinutes', 30))
    ...
    logger.debug(f"Pulse status for agent_id={agent_id}: enabled={pulse_enabled}, interval={pulse_interval}min")
